...
class FitnessEvaluate(object):
    ...
    def evaluate(self):
        ...
        _map = Utils.load_cache_data() # first time is empty
        _count = 0
        for indi in self.individuals:
            _key, _str = indi.uuid()
            if _key in _map:
                _count += 1
                _performance = _map[_key]
                self.log.info('Hit the cache for %s, key:%s, inv_acc:%.5f, spike_num:%.5f, crowd_distance:%.5f'%(indi.id, _key, float(_performance[0]), float(_performance[1]), float(_performance[2])))
                indi.inv_acc = float(_performance[0])
                indi.spike_num = float(_performance[1])
                indi.crowd_distance = float(_performance[2])
        ...
        for indi in self.individuals:
            if indi.inv_acc < 0:
                ...
                file_name = indi.id
                self.log.info('%s has inherited the fitness as inv_acc:%.5f, spike_num:%.5f, crowd_distance:%.5f no need to evaluate'%(file_name, indi.inv_acc, indi.spike_num, indi.crowd_distance))
                f = open('./populations/after_%s.txt'%(file_name[4:6]), 'a+')
                f.write('%s=%.5f,%.5f,%.5f\n' % (file_name, indi.inv_acc, indi.spike_num, indi.crowd_distance))
                f.flush()
                f.close()
        ...
        20220816 add dynamic GPU list when other occupy the GPUs
        """
        ...
        if has_evaluated_offspring:
            ...
            for line in f:
                if len(line.strip()) > 0:
                    ...
                    fitness_map[line[0]] = [float(_performance[0]), float(_performance[1]), float(_performance[2])]
            f.close()
            for indi in self.individuals:
                if indi.inv_acc == -1:
                    if indi.id not in fitness_map:
                        ...
                        sleep(120)
                    indi.inv_acc = float(fitness_map[indi.id][0])
                    indi.spike_num = float(fitness_map[indi.id][1])
                    indi.crowd_distance = float(fitness_map[indi.id][2])
        else:
