...
class CrossoverAndMutation(object):
    def __init__(self, prob_crossover, prob_mutation, _log, individuals, pops, _params=None):
        self.pops = pops
    ...
    def process(self):
        crossover = Crossover(self.individuals, self.prob_crossover, self.log, self.pops)
...
class Crossover(object):
    def __init__(self, individuals, prob_, _log, pops):
        self.individuals = individuals
        self.prob = prob_
        self.log = _log
    ...
    def _generate_parent_list(self):
        nsga = NSGAII()
        self.pops.front = nsga.fast_nodominate_sort(self.individuals)
        for Fi in self.pops.front:
            nsga.crowding_dist(Fi, self.individuals)
        _parents = []
        _candidate_parent = []
        _candidate_parent_length = len(self.individuals)
        count_ = 0
        print('pops.front', self.pops.front)
        for each_front in self.pops.front:
            if count_ > _candidate_parent_length:
                break
            # TODO: Fi is not 1
            if len(each_front) == 1:
                _candidate_parent.append(each_front[0])
                count_ += 1
            elif len(each_front) > 1:
                for _ in range(int(np.ceil(len(each_front) / 2))):
                    idx1, idx2 = self._choose_two_diff_parents_front(each_front)
                    _candidate_parent.append(idx1)
                    _candidate_parent.append(idx2)
                    count_ += 2
            else:
                raise TypeError('No define front length!!')

        for i in range(0, len(_candidate_parent), 2):
            _parents.append(_candidate_parent[i:i + 2])
        print(_parents)
        return _parents

    def _choose_one_parent_front(self, Fi):
        count_ = len(Fi)
        idx1 = Fi[random.randint(0, count_ - 1)]
        idx2 = Fi[random.randint(0, count_ - 1)]
        while idx2 == idx1:
            idx2 = Fi[random.randint(0, count_ - 1)]

        if self.individuals[idx1].crowd_distance > self.individuals[idx2].crowd_distance:
            return idx1
        else:
            return idx2

    def _choose_two_diff_parents_front(self, Fi):
        idx1 = self._choose_one_parent_front(Fi)
        idx2 = self._choose_one_parent_front(Fi)
        while idx2 == idx1:
            idx2 = self._choose_one_parent_front(Fi)
    ...
    def do_crossover(self):
        ...
        new_offspring_list = []
        _candidate_parent = self._generate_parent_list()
        for indi_num in range(len(self.individuals)//2):
            ind1 = _candidate_parent[indi_num][0]
            front_len = len(_candidate_parent[indi_num])
            if front_len == 1:
                ind2 = ind1
            elif front_len == 2:
                ind2 = _candidate_parent[indi_num][1]
            else:
                raise TypeError('indi length not available!!!')
            parent1, parent2 = copy.deepcopy(self.individuals[ind1]), copy.deepcopy(self.individuals[ind2])
            p_ = random.random()
            if p_ < self.prob: # 0.9 crossover
                ...
                offspring1.reset_performance()
                offspring2.reset_performance()
...
class Mutation(object):
    ...
    def do_mutation(self):
        print('do mutation')
        _stat_param = {'offspring_new':0, 'offspring_from_parent':0, 'ADD':0, 'REMOVE':0, 'CHANNEL':0, 'BACKWARD':0}

        mutation_list = StatusUpdateTool.get_mutation_probs_for_each() # mutation_probs = 0.7, 0.1, 0.1, 0.1
        for indi in self.individuals:
            p_ = random.random()
            if p_ < self.prob: # 20% mutation
                ...
                if mutation_type == 0:
                    ...
                    _stat_param['BACKWARD'] += 1
                    self.do_add_backward_connection(indi)
                else:
                    raise TypeError('Error mutation type :%d, validate range:0-4'%(mutation_type))
            else:
                _stat_param['offspring_from_parent'] += 1
        self.log.info('MUTATION-mutated individuals:%d[ADD:%2d,REMOVE:%2d,CHANNEL:%2d,BACKWARD:%2d, no_changes:%d'%(_stat_param['offspring_new'], \
                      _stat_param['ADD'], _stat_param['REMOVE'], _stat_param['CHANNEL'], _stat_param['BACKWARD'],  _stat_param['offspring_from_parent']))


    def do_add_backward_connection(self, indi):
        self.log.info('ADD the BACKWARD connection for indi:%s'%(indi.id))
        mutation_position = int(np.floor(np.random.random() * len(indi.units)))
        print('Mutation position occurs at %d'%(mutation_position))
        self.log.info('Mutation position occurs at %d'%(mutation_position))
        if indi.units[mutation_position].type == 1:
            indi.units[mutation_position].backward = True
            indi.reset_performance()
    ...
    def do_add_unit_mutation(self, indi):
        ...
        #do the details
        if type_ == 2:
            add_unit = indi.init_a_pool(mutation_position+1)
        else:
            for i in range(mutation_position, -1, -1):
                if indi.units[i].type == 1:
                    _in_channel = indi.units[i].out_channel
                    break
            add_unit = indi.init_a_lifnode(mutation_position+1, _in_channel=_in_channel, _out_channel=None, _fire=True, _backward=False)
...
#     indis = population.test_population()
#     m = Mutation(indis.individuals, 1, None)
#     m.do_mutation()
